...
interface ViewInheritedModalProps {
  isOpen: boolean;
  onClose: () => void;
  owner: string;
...
export function ViewInheritedModal({ isOpen, onClose, owner, credentialIndex }: ViewInheritedModalProps) {
  ...
  // Get credential name
  const { data: credentialName } = useReadContract({
    ...heirloomContract,
    functionName: "getCredentialName",
    args: [owner as `0x${string}`, BigInt(credentialIndex)],
    query: { enabled: isOpen && !!owner },
  });
  ...
  const handleDecrypt = async () => {
    if (!publicClient || !instance || !walletClient) {
    ...
    setTimeout(async () => {
      try {
        ...
        const result = await publicClient.readContract({
          ...heirloomContract,
          functionName: "getAuthorizedCredentialHandles",
          args: [owner as `0x${string}`, BigInt(credentialIndex)],
          account: address,
        }) as [string, string, string, string];

        const [accountHandle, passwordHandle, extra1Handle, extra2Handle] = result;
        console.log("[Inherited] Handles:", { accountHandle, passwordHandle, extra1Handle, extra2Handle });
  ...
  const name = (credentialName as string) || `Credential #${credentialIndex}`;
      ...
      <div className="relative w-full max-w-lg mx-4 bg-zinc-950 border border-zinc-800">
        <div className="flex items-center justify-between p-6 border-b border-zinc-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500">From: {shortOwner}</p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-4">
              ...
              <div className="w-full bg-zinc-800 border border-zinc-700 px-4 py-3 text-white min-h-[48px] flex items-center">
                {isDecrypted ? (
                  <span className="font-mono">
                    {showPassword ? decryptedData.password || "-" : "•".repeat(12)}
                  </span>
                ) : (
                  <span className="text-zinc-600 select-none">{"*".repeat(16)}</span>
                )}
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <div className="w-full bg-zinc-800 border border-zinc-700 px-4 py-3 text-white min-h-[72px]">
                {isDecrypted ? (
        ...
        <div className="p-6 pt-0">
          {!isDecrypted ? (
            <button
              ...
              disabled={decrypting}
              className="w-full py-3 bg-blue-500 text-white font-bold uppercase tracking-wide hover:bg-blue-400 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {decrypting ? (
                <>
